...
static void wake_due_sleepers_locked(unsigned int local_core){
    while (1){
        ...
        if (owner_core < MAX_CPU_CORES && current_pid[owner_core] == wake_pid){
            // The task is sleeping inside its own syscall frame on this core.
            // Do not enqueue a second runnable copy of the same PID; the
            // process_sleep() loop will observe READY and return to EL0.
