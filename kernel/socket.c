...
#define SOCKET_STREAM_RX_CAP 262144u
...
int ksocket_send(int pid, int fd, const unsigned char* data, unsigned int len, unsigned int flags){
    ...
    if (type == QOS_SOCK_STREAM){
        ...
        if (stream_remote_port == 443u){
            uart_puts("HTTPS profile: kex_used=X25519, kex_pq=off, x509_hostname=on, x509_chain_anchor=on, sig_advertised=RSA_PSS/ECDSA/ED25519/MLDSA65(draft)\n");
