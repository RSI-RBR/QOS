...
int dma_memcpy_2d(void* dst,
    ...
    g_dma_cb.ti = DMA_TI_TDMODE |
                  DMA_TI_WAIT_RESP |
                  DMA_TI_DEST_INC |
                  DMA_TI_SRC_INC |
                  DMA_TI_NO_WIDE_BURSTS;
    g_dma_cb.source_ad = dma_bus_address(src);
    g_dma_cb.dest_ad = dma_bus_address(dst);
    g_dma_cb.txfr_len = (rows << 16) | row_bytes;
    g_dma_cb.stride = ((dst_stride & 0xFFFFu) << 16) | (src_stride & 0xFFFFu);
